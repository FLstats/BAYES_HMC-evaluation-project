...
fit <- readRDS(file.path(getwd(), "stanfits", "hlr_fit_mlvl_a-ncp_b-ncp_146p.rds"))
...
groups <- draws %>%
  ...
  group_by(g) %>%
  median_qi(.value, .width = c(.5, .95))

# Caterpillar plot
groups %>%
  filter(.width == 0.95) %>%
  mutate(g = reorder(as.factor(g), .value)) %>%
  ggplot(aes(y = g, x = .value, xmin = .lower, xmax = .upper)) +
  # geom_pointrange(color = cbbPalette[6]) +
  geom_errorbar(width = 0.1) +
  geom_point(size = 2, color = cbbPalette[6]) +
  labs(x = "Group-intercept effect (log-odds)", y = "Group")
